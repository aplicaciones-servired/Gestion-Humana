...
import { Herraminetas, initHerraminetasModel } from "../Models/Herraminetas.models";
...
export const getHerramientas = async (req = request, res = response) => {
  ...
  try {
    const herramientas = await Herraminetas.findAll({
      where: whereClause,
      limit: pageSize,
      offset: offset,
      order: [
    ...
    const count = await Herraminetas.count({ where: whereClause });
    res.status(200).json({ count, datos: herramientas, page, pageSize });
  } catch (error) {
    res.status(500).json({ message: "Error retrieving herramientas", error });
  }
};