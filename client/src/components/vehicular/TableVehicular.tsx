...
import { useFiltersVehicular } from "@/Hooks/useFiltersVehicular";
import DialogVehicular from "./DialogVehicular";

interface TableVehicularProps {
    DataVehicular: Vehicular[];
    pagination?: React.ReactNode;
}

const TableVehicular = ({ DataVehicular, pagination }: TableVehicularProps) => {
    const { filteredVehicular, searchfecha, setSearchFecha } = useFiltersVehicular(DataVehicular);
    const [selectedItem, setSelectedItem] = useState<Vehicular | null>(null);
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {filteredVehicular.length === 0 ? (
                                    ...
                                    filteredVehicular.map((vehicular) => (
                                        <tr
                                            key={vehicular.id}
                                            onClick={() => {
                                                setOpen(true);
                                                setSelectedItem(vehicular);
                                            }}
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-4 text-sm text-gray-700 whitespace-nowrap">
                                                {vehicular.kilometraje}
                                            </td>
                                            <td className="px-4 py-4 text-sm text-gray-700 whitespace-nowrap">
                                                <span className={`px-2 py-1 text-xs rounded-full ${vehicular.l_baja === 'Conforme' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                                    }`}>
                                                    {vehicular.l_baja}
                                                </span>
                                            </td>
                                            <td className="px-4 py-4 text-sm text-gray-700 whitespace-nowrap">
                                                <span className={`px-2 py-1 text-xs rounded-full ${vehicular.principal === 'Conforme' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                                    }`}>
                                                    {vehicular.principal}
                                                </span>
                                            </td>
